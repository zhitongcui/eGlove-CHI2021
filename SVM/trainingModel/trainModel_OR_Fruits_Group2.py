...
def trainSVM():
    ...
    df1 = loadFile('data/OR_Fruits/Fruits_Orange.csv')
    df2 = loadFile('data/OR_Fruits/Fruits_Date.csv')
    # df3 = loadFile('data/OR_Fruits/Fruits_Banana.csv')
    df4 = loadFile('data/OR_Fruits/Fruits_Corn.csv')
    df5 = loadFile('data/OR_Plants/Fruits_Greenbasket.csv')
    df6 = loadFile('data/OR_Fruits/Fruits_Kiwi_Berry.csv')

    # 将所有数据合在一起
    df_alldata = pd.concat([df0, df1, df2, df4, df5, df6], axis=0, ignore_index=True)
    print(df_alldata)
