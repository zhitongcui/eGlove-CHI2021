...
def loadFile(path):
    ...
    # index_col=0将第一列作为索引
    df = pd.read_csv(path, index_col=0, converters={'Objects': typeConverter})
...
def typeConverter(type):
    # 字符串前面加 'r'，表示这是个普通字符串
    fruits_type = {r'Soundbox': 0,
                   r'Glasses': 1,
                   r'iPhoneXR': 2,
                   r'Key': 3,
                   r'Popcan': 4,
                   r'Plastic_full_water': 5,
                   r'AirPods_Chargebox': 6,
                   r'Stainless_Cup_No_Water': 7,
                   r'Stainless_Cup_Full_Water': 8,
                   r'Bowl': 9,
                   r'Nothing': 10}
...
def objectConverter(type):
    fruits_type = {0: r'Soundbox',
                   1: r'Glasses',
                   2: r'iPhoneXR',
                   3: r'Key',
                   4: r'Soundbox',
                   5: r'Plastic full water',
                   6: r'AirPods Charging Box',
                   7: r'Stainless Cup',
                   8: r'Stainless Cup',
                   9: r'Bowl',
                   10: r'Nothing'}
...
def trainSVM():
    ...
    # 从路径中读取csv数据
    df0 = loadFile('data/OR_Daily/Objects_Soundbox.csv')
    df1 = loadFile('data/OR_Daily/Objects_Glasses.csv')
    # df2 = loadFile('data/OR_Daily/Objects_OnePlus_Phone.csv')
    df3 = loadFile('data/OR_Daily/Objects_iPhoneXR.csv')
    df4 = loadFile('data/OR_Daily/Objects_Key.csv')
    df5 = loadFile('data/OR_Daily/Objects_Popcan.csv')
    df6 = loadFile('data/OR_Daily/Objects_Plastic_full_water.csv')
    df7 = loadFile('data/OR_Daily/Objects_AirPods_Chargebox.csv')
    ...
    # 将所有数据合在一起
    df_alldata = pd.concat([df0, df1, df3, df4, df5, df6, df7, df8, df9, df10, df11], axis=0, ignore_index=True)
    print(df_alldata)
