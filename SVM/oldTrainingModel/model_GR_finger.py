...
def loadFile(path):
    ...
    # index_col=0将第一列作为索引
    df = pd.read_csv(path, index_col=0, converters={'Gesture': typeConverter})
...
def typeConverter(type):
    # 字符串前面加 'r'，表示这是个普通字符串
    fruits_type = {r'one_finger_top': 0,
                   r'one_finger_bot': 0,
                   r'two_finger_top': 1,
                   r'two_finger_bot': 1}
...
def dataConverter(type):
    fruits_type = {0: r'OneFinger',
                   1: r'TwoFinger',
                   2: r'Banana',
                   3: r'Cucumber'}
    return fruits_type[type]
...
def trainSVM():
    # 从路径中读取csv数据
    df0_top = loadFile('data/GR_finger/one_finger_top.csv')
    df0_bot = loadFile('data/GR_finger/one_finger_bot.csv')
    df1_top = loadFile('data/GR_finger/two_finger_top.csv')
    df1_bot = loadFile('data/GR_finger/two_finger_bot.csv')

    # 拼接两个表的数据
    df_onefinger = pd.concat([df0_top, df0_bot], axis=1, ignore_index=True)
    df_twofinger = pd.concat([df1_top, df1_bot], axis=1, ignore_index=True)

    # 从数据中删除多余的一列
    df_onefinger.drop(axis=1, columns=225, inplace=True)
    df_twofinger.drop(axis=1, columns=225, inplace=True)

    # 检查是否正确删除中间的一行数据，':'表示行全选，'[255]'表示第255列，inplace=True表示在原数据上继续删除
    # print(df_apple.iloc[:,[225]])

    # 将所有数据合在一起
    df_alldata = pd.concat([df_onefinger, df_twofinger], axis=0, ignore_index=True)
    ...
    # Stratified K-Folds cross-validator 分层交叉验证
    scores = cross_val_score(clf, X, y.values.ravel(), cv=10, scoring='accuracy')
    ...
    # 通过验证集，验证模型的准确率
    # df_test_top = loadFile('/Users/cuizhitong/Desktop/one_finger_top.csv')
    # df_test_bot = loadFile('/Users/cuizhitong/Desktop/one_finger_bot.csv')
